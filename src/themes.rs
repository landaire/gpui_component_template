...
pub fn init(cx: &mut App) {
    ...
    if let Ok(state) = serde_json::from_str::<State>(&json) {
        ...
        AppState::global_mut(cx).theme = Some(state.theme.clone());
        if state.theme == "default-light" {
            Theme::global_mut(cx).set_default_light();
        } else if state.theme == "default-dark" {
            Theme::global_mut(cx).set_default_dark();
        } else if let Some(theme) = THEMES.get(&state.theme) {
            Theme::global_mut(cx).apply_config(theme);
        }
    }
}

pub fn save_theme(theme_name: SharedString) {
    let state = State { theme: theme_name };
    ...
    fn render(
        ...
        div()
            ...
            .on_action(cx.listener(|this, switch: &SwitchTheme, _, cx| {
                ...
                AppState::global_mut(cx).theme = Some(theme_name.clone());
